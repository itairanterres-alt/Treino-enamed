...
export const supabaseConfigurationError=Boolean(url||publishable)&&(!isHttpUrl(url)||!publishable);
export const supabase=!supabaseConfigurationError&&url&&publishable?createClient(url,publishable):null;

function readDemoUser():AppUser|null{
 try{
  const raw=localStorage.getItem('treino-demo-user');
  return raw?JSON.parse(raw) as AppUser:null;
 }catch{
  localStorage.removeItem('treino-demo-user');
  return null;
 }
}
...
export function AuthProvider({children}:{children:ReactNode}){
 ...
 useEffect(()=>{if(!supabase)return;supabase.auth.getUser().then(async({data})=>{if(data.user)setUser(await mapUser(data.user))}).catch(()=>undefined).finally(()=>setLoading(false));const {data}=supabase.auth.onAuthStateChange((_e,s)=>{if(!s?.user){setUser(null);return}void mapUser(s.user).then(setUser)});return()=>data.subscription.unsubscribe()},[]);
